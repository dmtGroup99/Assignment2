iteration10 <- function(data, train_instances){
  ...
  train_set$meanreview <- with(train_set, unlist(tapply(prop_review_score, prop_id, function(x) mean(x) - x))) + train_set$prop_review_score
  ...
  test_set$meanreview <- with(test_set, unlist(tapply(prop_review_score, prop_id, function(x) mean(x) - x))) + test_set$prop_review_score

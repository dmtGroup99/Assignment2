iteration20 <- function(data, train_instances){
  ...
  #IMPUTATIONS: review_score (kNN), prop_location_score2 (log lm), visitor_hist_starrating (lm), visitor_hist_adr_usd (lm)
  source('Variable_imputation.R')
  #train <- impute_review_score(train) #If this one does not work, then use impute_review_score_k
  train <- impute_review_score_k(train, k=25) #Perform kNN in k steps (The lower the k, the better)
  train <- impute_prop_location_score2(train)
  train <- impute_vist_hist(train, var = 'visitor_hist_starrating', th_corr = 0.10)
  train <- impute_vist_hist(train, var = 'visitor_hist_adr_usd', th_corr = 0.10)
  #test <- impute_review_score(test) #If this one does not work, then use impute_review_score_k
  test <- impute_review_score_k(test, k=10) #Perform kNN in k steps (The lower the k, the better)
  test <- impute_prop_location_score2(test)
  test <- impute_vist_hist(test, var = 'visitor_hist_starrating', th_corr = 0.10)
  test <- impute_vist_hist(test, var = 'visitor_hist_adr_usd', th_corr = 0.10)
